...
\begin{figure}[htbp]
    ...
    \caption{Simulated dynamics of the Hodgkin-Huxley model. The plots illustrate the temporal evolution of the transmembrane potential $V(t)$ along with the gating variables $m, h$, and $n$ under the influence of an applied external current $I_{app}(t)$.}
...
Since our setup of chapter \ref{Chpt3} operates on functions we considered the square wave perturbation class as it was the easiest to study.

Starting from these premises, we will turn to study the spatial extension of the FH model and analyze its response to a square wave perturbation localized also in space (i.e. applied to a single node).
...
To this end, we formulate a spatially extended system characterized by a specific input-output mapping. An external stimulus, modeled as a square-wave current, is injected into a designated \textcolor{blue}{\textbf{input node}}. The signal then propagates through the network via diffusive coupling, while each node undergoes local reaction dynamics governed by the FHN equations. Finally, the response of the system is recorded by extracting the membrane potential of a designated \textcolor{red}{\textbf{output node}}. This setup is illustrated by two examples in figure \ref{fig:fh_comparison}.
