...
One of the most notorious ionic models is the Hodgkin-Huxley (HH) model. It was first introduced by Alan Hodgkin and Andrew Huxley in 1952 \cite{hodgkin1952quantitative}, who received the 1963 Nobel Prize in Physiology or Medicine for this work.
\newline
...
This model is realistic, but somewhat convoluted for our objectives. In fact, our primary aim (as discussed by subsequent chapters) is being able to simulate a network of neurons and emulate it through ML. To this end, we need a system that is as simple as possible, while exhibits, like a physically realistic neuron, oscillations for currents that exceed a critical threshold
...
Starting from these premises, we will turn to study the spatial extension of the FH model and analyze its response to a square wave perturbation localized in space (i.e. applied to a single node).
...
To this end, we consider a spatially extended system characterized by a specific input-output mapping. An external stimulus, modeled as a square-wave current, is injected into a designated \textcolor{blue}{\textbf{input node}}. The signal then propagates through the network via diffusive coupling, while each node undergoes local reaction dynamics governed by the FHN equations. Finally, the response of the system is recorded by extracting the membrane potential of a designated \textcolor{red}{\textbf{output node}}. This setup is illustrated by two examples in figure \ref{fig:fh_comparison}.
