\chapter{Appendix: Numerical Integration 2}
\label{app:C}
...
\end{equation}
where, in out first example, $\mathbf{u} \in \mathbb{R}^N$ is the state vector, $\mathcal{L}$ is the graph Laplacian, $D=\epsilon^2$ is the diffusion coefficient, and $\mathbf{F}(\mathbf{u}) = \mathbf{u} - \mathbf{u}^3$ is the element-wise cubic reaction term.
