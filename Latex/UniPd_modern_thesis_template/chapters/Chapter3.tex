...
\begin{figure}[htbp]
    ...
    \caption{Four instances of the test set. For each column we can see the Input current (bottom) with its corresponding potential response (top): the blue line represents the result of the simulations, whereas the red dots indicate some outputs of the NN}
...
Consistent with the findings of the previous section, the network proves to be highly capable of capturing the underlying threshold dynamics. Figure \ref{fig:cycle_peaks} illustrates some instances of the test set for this new configuration. As we can see, the predicted dynamics strictly follows the numerical simulations, successfully reproducing the spiking behavior and the delay introduced by the diffusion over the cycle.
...
\begin{figure}[htbp]
    ...
    \caption{\textbf{(a)} Schematic representation of the network setup in the case of a simple (cyclical) hidden network. \textbf{(b)} Four instances of the test set for the simple cycle configuration. For each column we can see the applied input current (bottom) with its corresponding potential response (top): the blue line represents the result of the simulations, whereas the red dots indicate samples from the trained NN.}
...
%$=0,42,100$ 
to distinguish if some effects are only due to chance of the particular topology/ sample of input/ output.
...
\begin{figure}[htbp]
    ...
    \caption{Some instances of the test set for different Erd\H{o}s-R\'enyi network sizes ($N$).  For each column we can see the applied input current (bottom) with its corresponding potential response (top): the blue line represents the result of the simulations, whereas the red dots indicate the output of the NN.}
...
Since the input and output nodes are randomly chosen, we repeated each experiment with different three seeds 
%($0, 42, 100$) 
to distinguish whether specific effects are intrinsic to the topology or merely due to the chance of the particular sampling of input/output nodes.
...
\begin{figure}[htbp]
    ...
    \caption{Some instances of the test set for different Barab\'asi-Albert network sizes ($N$). For each column we can see the applied input current (bottom) with its corresponding potential response (top): the blue line represents the result of the simulations, whereas the red dots indicate the output of the NN.}
...
%($S=0,42,100$).
\newline
As the network size $N$ increases, we don't observe a significant error distribution spread toward higher values. This remarkable scalability may also be due to the discretization-invariant nature of the FNO architecture, since, as we saw in chapter \ref{Chp:2}, the learned architecture parameters do not depend on the specific number of nodes of the network.
...
\begin{figure}[htbp]
    ...
    \caption{Overview of the \texttt{mc\_maze} dataset and the behavioral decoding task. The upper panels illustrate the delayed center-out reaching task, displaying the 2D spatial trajectories of the macaque's hand navigating around varying virtual obstacles to reach specific targets. The lower panels display the corresponding neural population activity (spike raster plots) recorded from the M1 and PMd cortices.}
