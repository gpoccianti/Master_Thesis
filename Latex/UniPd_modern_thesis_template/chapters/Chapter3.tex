...
\begin{figure}[htbp]
    ...
    \caption{Four instances of the test set. For each column we can see the Input current (bottom) with its corresponding potential response (top): the continuous lines represent the result of the simulations, whereas the red dots indicate some outputs of the NN}
...
\begin{figure}[htbp]
    ...
    \caption{\textbf{(a)} Schematic representation of the network setup in the case of a simple (cyclical) hidden network. \textbf{(b)} Four instances of the test set for the simple cycle configuration. For each column we can see the applied input current (bottom) with its corresponding potential response (top): the continuous lines represent the result of the simulations, whereas the red dots indicate samples from the trained NN.}
...
\begin{figure}[htbp]
    ...
    \caption{Some instances of the test set for different Erd\H{o}s-R\'enyi network sizes ($N$).  For each column we can see the applied input current (bottom) with its corresponding potential response (top): the continuous lines represent the result of the simulations, whereas the red dots indicate the output of the NN.}
