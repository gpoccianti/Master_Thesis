...
As we could see in the previous graphs, and in the summary figure \ref{fig:fno_scaling_error} below, we can deduce that (at least for the sizes considered) the FNO generalizes well the deterministic dynamics. In fact, as the network size $N$ increases, we don't observe a significant error distribution spread toward higher values. This remarkable scalability may also be due to the discretization-invariant nature of this architecture, since, as we saw in chapter \ref{Chp:2}, the learned architecture parameters do not depend on the specific number of nodes of the network.
...
In fact, within the INPUT->network-> OUTPUT setup, not all path are equals, since the system will relax only when the signal will propagate to the OUTPUT node. This means that the shortest path from INPUT to OUTPUT becomes the most relevant for the behavior of spectral gap and it this alter its behavior.
