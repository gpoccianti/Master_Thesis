\chapter{Chapter 3}
\label{Chpt3}
In this chapter we will first present some previous work on the ML emulation of ionic systems. We will then build on these results to tackle a more complex, spatially extended problem, probing different network topologies. Finally, we will tackle a real neurological dataset and see how our emulation performs in that case.

\section{Problem definition}
\label{chpt: Problem definition}
In this work we expand on the results presented in \cite{centofanti2024comconf}, and developed in \cite{centofanti2024cma} and \cite{pellegrini2025mlcse}. In these articles an operator learning architecture (with a particular focus on Fourier Neural Operator (FNO) \footnote{See Chapter \ref{Chp:2}.}) has been used to learn the dynamics of a neuronal system, described by a ionic model (the FitzHugh-Nagumo or Hodgkin-Huxley model \footnote{See Chapter \ref{Chp:1}.}). 
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1.0\textwidth]{img/FH_model/No_Network/Centofanti_article_figure.png}
...
Please note the setup is the following \footnote{Unless otherwise specified.}: the \textcolor{blue}{\textbf{input node}} is connected by a direct link to a random node of a network. The hidden network is simple.
%Devo spiegare undirected + unweighted ?
Finally another random node is connected by another direct link to the \textcolor{red}{\textbf{output node}}.
%Questa sezione l'ho scritta prima dei capitoli introduttivi in cui spiego la stessa cosa. La tengo in quanto un recap può essere utile a chi legge solo questa sezione?
...
\begin{figure}[htbp]
    ...
    \caption{Four instances of the test set. For each column we can see the Input current (bottom) with its corresponding potential response (top): the continuous lines represent the results of the simulations, whereas the red dots indicate some outputs of the NN}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        \caption{MSE error distribution. As descriptors, we reported the median, 10th and 90th percentile}
...
Following the same approach, we evaluated the FNO performance on a more complex spatial configuration, introducing a simple cycle topology (see fig. \ref{fig:cycle_setup}). 

Consistent with the findings of the previous section, the network proved to be highly capable of capturing the underlying threshold dynamics. Figure \ref{fig:cycle_peaks} illustrates some instances of the test set for this new configuration. As we can see, the predicted dynamics strictly follows the numerical simulations, successfully reproducing the spiking behavior and the delay introduced by the diffusion over the cycle.

The global error metrics exhibits a behavior analogous to the "no network" scenario. As shown in Figure \ref{fig:cycle_performance}a, the training and test loss converge smoothly without any signs of overfitting. Furthermore, the MSE distribution across the test set (Figure \ref{fig:cycle_performance}b) remains tightly concentrated at very low values. This confirms that the operator learning approach is robust and its accuracy does not degrade when dealing with a small ($4$ nodes) network diffusion.

% --- Figura singola (Picchi Ciclo) ---

% Ricordati di includere \usepackage{subcaption} nel preambolo se non l'hai già fatto

\begin{figure}[htbp]
    \centering
    % Prima sottofigura (Ora all'80% della pagina)
    \begin{subfigure}[b]{0.8\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/FH_model/Simple_cycle/dynamics_cycle.png}
        \caption{Network schematic setup.}
        \label{fig:cycle_setup}
    \end{subfigure}
    
    \vspace{0.5cm} % Aggiunge un po' di spazio verticale tra le due figure
    
    % Seconda sottofigura (Ora all'80% della pagina)
    \begin{subfigure}[b]{0.8\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/FH_model/Simple_cycle/HHpeaks.png}
        \caption{Test set instances.}
        \label{fig:cycle_instances}
    \end{subfigure}
    
    \caption{\textbf{(a)} Schematic representation of the network setup in the case of a simple (cyclical) hidden network. \textbf{(b)} Four instances of the test set for the simple cycle configuration. For each column we can see the applied input current (bottom) with its corresponding potential response (top): the continuous lines represent the results of the simulations, whereas the red dots indicate samples from the trained NN.}
...
To conduct this experiment we performed our analysis for different sizes of the ER network $N= 10,100,200,300,400,500,600,700,800,900,1000,2000$ (with connection probability $p = \frac{3}{N}$).
Since the connection node for the input and output node are randomly chosen we repeated each experiments with three different seeds  
%$=0,42,100$ 
to distinguish if some effects were only due to chance of the particular topology/ sample of input/ output.

%Here we reported only figures for the seed $S=42$.

As a representative example of the system's behavior, Figure \ref{fig:er_setup_100} reports the potential dynamics for a network of size $N=100$. 
%generated with seed $100$.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{img/FH_model/ER_Network/dynamics_network_ER_N=100_seed=42_v2.png}
    \caption{Our setup for an Erd\H{o}s-R\'enyi network of size $N=100$.}
    \label{fig:er_setup_100}
\end{figure}

%%%
To systematically evaluate the threshold dynamics across all network dimensions, Figure \ref{fig:er_peaks_grid} reports the error distribution regarding the predicted number of action potential peaks for each size $N$. As we can observe from this comprehensive grid, the FNO robustly maintains its high accuracy: the vast majority of test instances exhibit zero error in the spike count, regardless of the significant increase in the network dimensionality. This is supported by looking at the error distributions in Figure \ref{fig:er_histo_grid} and loss curves in Figure \ref{fig:er_loss_grid}, where we can observe respectively a consistent small MSE and a good convergence behavior. Moreover this confirms that the model's ability to capture the exact spiking behavior is not compromised by the growing scale of the spatial diffusion.
...
\begin{figure}[htbp]
    ...
    \caption{Some instances of the test set for different Erd\H{o}s-R\'enyi network sizes ($N$).  For each figure's column we can see the applied input current (bottom) with its corresponding potential response (top): the continuous lines represent the results of the simulations, whereas the red dots indicate the output of the NN.}
...
%Vanno bene ? alcune cifre sull'asse x si sovrappongono e forse avrei dovuto usare la notazione scientifica per Mean=. ???
% ==========================================
...
Since the input and output nodes are randomly chosen, we repeated each experiment with three different seeds 
%($0, 42, 100$) 
to distinguish whether specific effects were intrinsic to the topology or merely due to the chance of the particular sampling of input/output nodes.
...
By looking at Figures \ref{fig:ba_peaks_grid}, \ref{fig:ba_histo_grid} and \ref{fig:ba_loss_grid} we can state, as for the case of ER network, that the performances of the FNO don't seem to degradate with the growing network size $N$ and that there is a good general tendency in capturing spiking behavior and model's response to external stimuli. 
...
\begin{figure}[htbp]
    ...
    \caption{Some instances of the test set for different Barab\'asi-Albert network sizes ($N$). For each column we can see the applied input current (bottom) with its corresponding potential response (top): the continuous lines represent the results of the simulations, whereas the red dots indicate the output of the NN.}
...
As we could see in the previous graphs (at least for the sizes considered) the FNO generalizes well the deterministic dynamics. 
These results are strengthened and summarized in the violin plots Figure \ref{fig:fno_scaling_error}, for the distribution of $\log(MSE)$, as network size and type vary. To gain statistical relevance we aggregated data for the three different seed employed.
%($S=0,42,100$).
\newline
As the network size $N$ increases, we don't observe a significant error distribution spread toward higher values. 
%This remarkable scalability may also be due to the discretization-invariant nature of the FNO architecture, since, as we saw in chapter \ref{Chp:2}, even if the neural network had had access to all the hidden variables, the learned architecture parameters would not have depended on the specific number of nodes in the network.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1.0\textwidth]{img/FH_model/error_FNO_scalingN_v2.png}
    \caption{Violin plots of the FNO error distribution across different network sizes ($N$). The dotted lines represent (respectively, from bottom to top) the first quartile, median and third quartile of each distribution. The plot compares the error scaling for both Erd\H{o}s-R\'enyi (ER) and Barab\'asi-Albert (BA) topologies, demonstrating the robustness and scalability of the architecture.}
...
In fact, within the INPUT->network-> OUTPUT setup, not all paths are of equal relevance, since the system will completely relax only when the signal will propagate to the OUTPUT node. This means that the shortest path from INPUT to OUTPUT becomes the most relevant for the behavior of spectral gap and this alter its behavior.
%By grounding the system and forcing the flow through a single input and a single output node, the boundary constraints effectively project the energy flow into a quasi-one-dimensional transport regime. 
As a result, our numerical spectral analysis demonstrates that the physical spectral gap $\text{Re}(\lambda_2)$ collapses, scaling exactly as $\mathcal{O}(N^{-1})$ for both the ER and BA setups, as it can be seen in Figure \ref{fig:spectral_gap_ba}-\ref{fig:spectral_gap_er}.
...
\begin{figure}[htbp]
    ...
    \begin{minipage}[t]{0.49\textwidth}
        ...
        \caption{Scaling of the spectral gap $\text{Re}(\lambda_2)$ as a function of $N$ for the Barabási-Albert (BA) topology, showing the $\mathcal{O}(N^{-1})$ decay, both for a standard and a network with preferential attachment of Input/Output to hubs.}
...
Therefore, to maintain a constant effective macroscopic coupling ($\sigma \text{Re}(\lambda_2) \approx \text{constant}$) and ensure that the operator learns a consistent continuous limit regardless of the spatial discretization size $N$, the physical diffusion coefficient must systematically scale as $\sigma \propto N$.
...
The possible choices for frequencies $f_1$ and $f_2$ are {10,
14, 18, 26, 30, 34}.
...
In our work, we replaced this standard linear decoder with our Operator Learning framework, which naturally operates on continuous function spaces. Specifically, we defined the input function $u(t)$ as the smoothed, continuous neural firing rates, and the target output function $v(t)$ as the continuous hand velocity of the macaque. By training the neural network to learn this operator (with FNO) $G: u \to v$, we inferred the continuous velocity profile directly from the underlying neural dynamics. Finally, the predicted velocitywas numerically integrated over time to reconstruct the full 2D reaching trajectory (position) of the hand. This setup allowed us to rigorously test the neural operator's ability to capture the complex, nonlinear mapping between cortical activity and physical behavior, moving beyond the limitations of simple linear regression.

\begin{figure}[htbp]
    \centering
    % Adjust the width if necessary (e.g., 0.9\textwidth or 1.0\textwidth)
    \includegraphics[width=0.8\textwidth]{img/Real_Data/maze_dataset.png}
    \caption{Overview of the \texttt{mc\_maze} dataset and the behavioral decoding task. The upper panels illustrate the delayed center-out reaching task, displaying the 2D spatial trajectories of the macaque's hand navigating around varying obstacles to reach specific targets. The lower panels display the corresponding neural population activity (spike raster plots) recorded from the M1 and PMd cortices.}
    \label{fig:mc_maze_dataset}
\end{figure}

\clearpage
\subsection{Preprocessing}
In our work, we replaced the standard linear decoder with our Operator Learning framework, specifically employing a Fourier Neural Operator (FNO), which naturally operates on continuous function spaces. As part of our preprocessing pipeline, the time series data was rigorously aligned and resampled at a 5 ms resolution to provide a strictly uniform temporal grid. The resulting dataset consists of 2295 individual reaching trajectories.
%, each standardized to a fixed temporal window of 500 ms (represented by 100 discrete time bins).
 We defined the input function $u(t)$ as the continuous neural firing rates from $182$ neural units, which have been smoothed using a Gaussian kernel ($\sigma = 50$ ms) to reduce point-process noise. The target output function $v(t)$ was defined as the continuous hand velocity of the macaque along the $x$ and $y$ axes. To account for the physiological delay required for signals to travel from the motor cortex to the muscles, we applied an $80$ ms lag to the kinematic data relative to the neural activity. By training the FNO to learn this operator $G: u \to v$, we inferred the continuous velocity profile directly from the underlying neural dynamics. Finally, the predicted velocity was numerically integrated over time to reconstruct the full 2D reaching trajectory (position) of the hand.
...
In Figure \ref{fig:loss_curves} we can see the training and validation curves for both types of architectures with the relative best epoch. We observe that the FNO converges faster than LSTM to an optimal solution. 
...
\midrule
Ridge Regression & 0.65 & 0.70 / 0.60 & 0.59 $\pm$ 0.14 & 0.57 & $\left(3.4 \pm 1.8\right)10^{5}$ \\
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{1.0\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/Real_Data/histogram_test_set_ridge_regression_V2.png}
