...
\end{equation}
For Level 3, the target was the approximation $\mathbf{{R}_{GT}}^{n}_{p} = \mathbf{\Phi}_p^T \mathbf{R}_{GT}^n \mathbf{\Phi}_p$, or the projection onto the spectral domain: $\hat{\mathbf{R}}_{GT}^n = \mathbf{\Phi}_p^T \mathbf{R}_{GT}^n$.
...
\end{equation}
where $\gamma$ is a balancing hyperparameter \footnote{in our experiments, we tuned $\gamma$ as an hyperparameter of the model, before training.}
...
we still exploited the Laplacian operator in the integration scheme. So, if we don't have access to the topological features 
we would need another integration method, which probably would lower the perfomances.

Finally it is worth mentioning the FNO architectures outperformed the DeepONet architectures at the point that, FNO outperformed leno.
...
After this first implementation we asked ourselves two questions: could we generalize this architecture to more than one variable ? 
...
To quantitatively assess the generalization capabilities of the trained model, we evaluated it on a strictly held-out test set. The histogram of the relative $L_2$ errors (Figure \ref{fig:lfno_hist}) exhibits a highly right-skewed distribution, indicating consistency across test samples. The model achieves an outstanding average relative $L_2$ error of just $(0.8 \pm 0.4)\%$, confirming that the LFNO has successfully learned the continuous operator mapping rather than merely memorizing the training trajectories.
