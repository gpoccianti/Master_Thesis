...
\begin{enumerate}
    ...
    \item $\mathcal{A} = C(\overline{D}) = \left\{ f: \overline{D} \to \mathbb{R} \mathrel{\Big|} f \text{ is continuous on the closure of } \overline{D} \right\}$, where $\overline{D}$ represents the closure of D.
\end{enumerate}
\end{assumption}
...
Among the emerging approaches, two architectures, based on different universal approximation theorems for operators, stand out: the Deep Operator Network (DeepONet) \cite{lu2021learning} and Neural Operators \cite{kovachki2023neural}. Within the last class, in particular, integral kernel-based methods such as the Fourier Neural Operator (FNO) \cite{li2020fourier} have demonstrated remarkable capabilities in learning non-linear operators on regular or equispaced domains.
