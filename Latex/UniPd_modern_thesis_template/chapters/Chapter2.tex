...
In this chapter we will first give a brief intoduction to Machine Learning, then we will describe the main approximation theorems for Neural Networks, with a special focus on Operator Learning.
With this knowledge we will then delve into the different branches of Scientific Machine Learning and describe the most relevant neural operators architectures.

\section{A brief introduction to Machine Learning}
Machine Learning (ML) encompasses a broad set of methods that enable algorithms to learn complex patterns and relationships from data. While traditional approaches for data analysis rely on manually engineering features, ML techniques automatically adjust their internal parameters to improve performance on a specific task. Here we specifically focus on the application of Deep Learning, characterized by the utilization of Artificial Neural Networks (ANNs).
...
The foundational theoretical work for operator approximation was established by Chen and Chen \cite{Chen_Chen_1995}, who demonstrated that a neural network with a specific architecture can approximate any continuous nonlinear operator. This theorem directly inspired the DeepONet architecture \cite{lu2021learning} (see Section \ref{DeepONet section}).
...
Suppose $\sigma$ is a continuous non-polynomial function. For any $\epsilon > 0$, there exist positive integers $n, m, p$, points $x_1, \dots, x_m \in K_1$, and constants $c_i^k, \xi_{ij}^k, \theta_i^k, \zeta_k \in \mathbb{R}$, $w_k \in \mathbb{R}^{d_2}$ (for $i=1,\dots,n$, $j=1,\dots,m$ and $k=1,\dots,p$) such that:
$$ \left| \mathcal{G}(u)(y) - \sum_{k=1}^p \sum_{i=1}^n c_i^k \sigma \left( \sum_{j=1}^m \xi_{ij}^k u(x_j) + \theta_i^k \right) \sigma(w_k \cdot y + \zeta_k) \right| < \epsilon $$
holds for all $u \in V$ and $y \in K_2$.
\end{theorem}

This theorem explicitly reflects the dual-network structure of DeepONet. The term $\sigma\left(\sum_{j=1}^m \xi_{ij}^k u(x_j) + \theta_i\right)$ inspired the \textit{branch net}, which takes discrete evaluations of the input function $u$ at sensor locations $x_j$. Conversely, the term $\sigma(w_k \cdot y + \zeta_k)$ suggested the \textit{trunk net}, which evaluates the continuous spatial or temporal coordinates $y$.
...
\begin{enumerate}
    \item $\mathcal{A} = L^{p_1}(D) = \left\{ f: D \to \mathbb{R} \text{ measurable } \mathrel{\Big|} \int_D |f(x)|^{p_1} \, dx < \infty \right\}$ for some $1 \le p_1 < \infty$;
    \item $\mathcal{A} = W^{m_1,p_1}(D) = \left\{ f \in L^{p_1}(D) \mathrel{\Big|} D^\alpha f \in L^{p_1}(D) \text{ for all multi-indices } \alpha \text{ with } |\alpha| \le m_1 \right\}$ for some $1 \le p_1 < \infty$ and $m_1 \in \mathbb{N}$, where $D^\alpha f$ denotes the weak partial derivative;
    \item $\mathcal{A} = C(\overline{D}) = \left\{ f: \overline{D} \to \mathbb{R} \mathrel{\Big|} f \text{ is continuous on } \overline{D} \right\}$, where $\overline{D}$ represents the closure of D.
\end{enumerate}
\end{assumption}
...
The theoretical architecture of the Neural Operator, denoted as $NO_T(\sigma_1, \sigma_2, \sigma_3; D, D')$, represents the class of neural operators where $T \in \mathbb{N}$ is the number of integral layers (the depth of the network), $\sigma_1, \sigma_2, \sigma_3$ are the continous, non polynomial, activation functions used respectively for the initial lifting mapping, the iterative kernel layers, and the final projection mapping, while $D$ and $D'$ are the input and output spatial domains. This architecture is specifically constructed so that its continuous layers can seamlessly approximate infinite-dimensional mappings with arbitrary precision. Note that for Fourier Neural Operator (see section \ref{DeepONet section}) $D=D'$.
...
Then, for any compact set $K \subset \mathcal{A}$ and any $\epsilon > 0$, there exists an integer $T \in \mathbb{N}$ and a Neural Operator $\mathcal{G} \in NO_T(\sigma_1, \sigma_2, \sigma_3; D, D')$ such that:
\[ 
...
An operator learning architecture is said to be discretization-invariant if the model parameters $\theta$ are defined in a continuous function space and can be applied to any discretization \footnote{Provided that there are enough sample points to perform a DFT with $k$ modes.} %Controllare la correttezza di questa affermazione.
%L'idea è che sopra un certo numero di punti posso fare
%DFT, tengo comunque i primi k coefficienti e quindi
%la struttura diventa indipendente dalla discretizzazione
%La parte W_t che agisce nello spazio diretto
%invece agisce su uno spazio fittizzio (i canali) e non ha alcuna dimensione dipendente dalla discretizzazione!
 of the input and output domains $D, D'$. 
...
\]
Because the kernel $\kappa$ (or its Fourier representation $R$ in Fourier Neural Operators) is a continuous object, the same parameters $\theta$ can be evaluated on a different set of points $\{x'_1, \dots, x'_k\}$ without retraining.
...
In the context of the Fourier Neural Operator (FNO, see section \ref{DeepONet section}) architecture, this discretization invariance stems from two structural choices. First, the learnable integral kernel $R$ is evaluated in the Fourier space where only a truncated, fixed number of low-frequency modes are preserved. When evaluating the model on a denser grid, the higher-frequency modes computed by the discrete Fourier transform are simply bypassed, keeping the size of $R$ constant. Second, the residual weight matrix $W$ operates strictly pointwise on the latent state's scalar components (channels), making it entirely independent of the spatial domain's dimensionality. Together, these properties ensure that the network's weights are perfectly agnostic to the underlying spatial discretization.
%From the construction of neural operators, when the input and output functions are evaluated on fixed grids, the architecture of neural operators on these fixed grids coincides with the class of neural networks.
...
As demonstrated in the numerical experiments for parametric PDEs in article \cite{li2020fourier}, the relative error of a discretization-invariant model remains consistent across different resolutions. This is formally expressed by the property that as the discretization becomes finer ($n \to \infty$), the discrete approximation converges to the continuous operator:
\[ 
\lim_{n \to \infty} \hat{\mathcal{G}}_{\theta}(D_n, a_n) = \mathcal{G}_{\theta}(a) 
\]
...
Figure \ref{fig:sciml_landscape} summarizes differences between these branches of SciML. The second area is related to direct problems and has proven effective in accelerating traditional solvers. 
\newline
...
Among the emerging approaches, two architectures, based on different universal approximation theorems for operators, stand out: the Deep Operator Network (DeepONet) \cite{lu2021learning} and Neural Operators \cite{kovachki2023neural}. Within the last class, in particular, the Fourier Neural Operator (FNO) \cite{li2020fourier} have demonstrated remarkable capabilities in learning non-linear operators on regular or equispaced domains.
...
\end{itemize}
The output of the approximated operator is obtained through the dot product of the outputs of these two networks, allowing the solution to be evaluated at any desired point $y$ of the domain \cite{lu2021learning}:
\begin{equation}
    \hat{G}_{\theta}(u)(y) = \sum_{k=1}^{p} \underbrace{b_k(u(x_1), \dots, u(x_m))}_{\text{Branch Net}} \cdot \underbrace{t_k(y)}_{\text{Trunk Net}}
\end{equation}

\begin{figure}[htbp]
    \centering %0.8
    \includegraphics[width=0.8\textwidth]{img/FH_model/DeepONet2.png}
    \caption{(a) Example of a training dataset, formed by sets of input functions $u$ sampled on a fixed domain grid $x_1,x_2,...,x_m$ and corresponding desired outputs $G(u)$, sampled at any desired point $y$ of the codomain. 
    \newline
    (b) Schematic of the DeepONet architecture. The \textit{branch net} processes the discretized input function, while the \textit{trunk net} processes the domain coordinates. Their outputs are combined via a dot product. Adapted from \cite{lu2021learning}.}
...
\begin{enumerate}
    \item \textbf{Lifting:} The input function $a(x)$ is lifted to a higher-dimensional hidden representation $v_0(x)$ via a local transformation $P$ (typically a shallow fully connected network with activation $\sigma_1$): \\
    $v_0(x) = P(a(x)) = \sigma_1(W_{\text{in}} a(x) + b_{\text{in}}), \quad x \in D.$
    
    \item \textbf{Iterative Kernel Updates:} For $t = 0, \dots, T-1$, the hidden representation is updated using a pointwise linear operator $W_t$, an integral kernel operator with parameterized kernel $\kappa_t(x,y)=\kappa_t(x-y)$, and the non-linear activation function $\sigma_2$: \\
    $\displaystyle v_{t+1}(x) = \sigma_2 \left( W_t v_t(x) + \int_{D} \kappa_t(x,y) v_t(y) \, dy + b_t \right).$
    
    \item \textbf{Projection:} The final hidden representation $v_T$ is projected back to the target space defined on $D'$ using another local transformation $Q$ \footnote{While represented here as a single-layer affine transformation for notational consistency, deeper configurations are often employed in practice.} (with activation $\sigma_3$): \\
    $\mathcal{G}(a)(x) = Q(v_T(x)) = \sigma_3(W_{\text{out}} v_T(x) + b_{\text{out}}), \quad x \in D.$
\end{enumerate}

In the specific case of the Fourier Neural Operator (FNO) \cite{li2020fourier}, the integral kernel is parameterized directly in the frequency domain \footnote{Other choices for the kernel are valid and lead to different architectures.}. By leveraging the convolution theorem, the continuous integral operation is efficiently resolved by computing the Fourier transform of the input, performing a pointwise multiplication with a learnable complex-valued weight tensor over a truncated set of low-frequency modes,
...
We are now ready to apply neural operators for the emulation of high-dimensional ionic dynamics, as anticipated in Chapter \ref{Chp:1}. 